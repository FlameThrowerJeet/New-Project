...
// Mock agent data
const mockAgents = [
  { name: 'अमन सिंह', photo: 'https://randomuser.me/api/portraits/men/32.jpg' },
  { name: 'प्रिया शर्मा', photo: 'https://randomuser.me/api/portraits/women/44.jpg' },
  { name: 'रवि कुमार', photo: 'https://randomuser.me/api/portraits/men/45.jpg' },
];
...
const CleaningServices: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: 480, margin: '40px auto', background: 'rgba(16,32,16,0.95)', borderRadius: 12, boxShadow: '0 0 24px #0f08', padding: 32, color: '#0f0' }}>
      <h2 style={{ textAlign: 'center', marginBottom: 24 }}>सफाई सेवाएं</h2>
      <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 24 }}>
        <button onClick={() => setTab('hire')} style={{ padding: '8px 24px', borderRadius: 6, border: 'none', background: tab === 'hire' ? '#0f0' : '#222', color: tab === 'hire' ? '#111' : '#0f0', fontWeight: 'bold', cursor: 'pointer', fontSize: 16 }}>एजेंट किराए पर लें</button>
        <button onClick={() => setTab('become')} style={{ padding: '8px 24px', borderRadius: 6, border: 'none', background: tab === 'become' ? '#0f0' : '#222', color: tab === 'become' ? '#111' : '#0f0', fontWeight: 'bold', cursor: 'pointer', fontSize: 16 }}>एजेंट बनें</button>
      </div>
      {tab === 'hire' ? (
        success ? (
          <div style={{ color: '#0f0', textAlign: 'center', fontWeight: 'bold', fontSize: 18 }}>
            बुकिंग पुष्टि हुई!<br />हमारा एजेंट जल्द ही आपसे संपर्क करेगा।<br /><br />
            <button style={{ marginTop: 16, background: '#0f0', color: '#111', border: 'none', borderRadius: 6, padding: '8px 18px', fontWeight: 'bold', cursor: 'pointer', fontSize: 16 }} onClick={() => setSuccess(false)}>और बुक करें</button>
          </div>
        ) : (
          <form onSubmit={e => { e.preventDefault(); setSuccess(true); }}>
            {/* Step 1: Date Picker */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 6 }}>1. तारीख चुनें:</label>
              {/* @ts-ignore */}
              <DatePicker
                selected={date}
            ...
            {date !== null && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', marginBottom: 8, color: '#0f0' }}>2. सत्र चुनें:</label>
            ...
            {date !== null && selectedSlot !== null && (
              <>
                <div style={{ marginBottom: 18 }}>
                  <label style={{ display: 'block', marginBottom: 6 }}>3. स्थान चुनें (मानचित्र पर क्लिक करें):</label>
                  ...
                  <input type="text" value={gps} onChange={e => setGps(e.target.value)} placeholder="GPS निर्देशांक (lat,lon)" required style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #0f0', background: '#111', color: '#0f0' }} />
                </div>
                {/* Step 3.1: Locality Selection */}
                {gps && (
                  <div style={{ marginBottom: 14 }}>
                    <label style={{ display: 'block', marginBottom: 6 }}>4. इलाका चुनें:</label>
                ...
                {gps && selectedLocality && (
                  <div style={{ marginBottom: 14 }}>
                    <label style={{ display: 'block', marginBottom: 6 }}>5. सड़क चुनें:</label>
                    <select value={selectedStreet} onChange={e => setSelectedStreet(e.target.value)} required style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #0f0', background: '#111', color: '#0f0' }}>
                      <option value="">सड़क चुनें</option>
                ...
                {gps && selectedLocality && selectedStreet && (
                  <div style={{ marginBottom: 14 }}>
                    <label style={{ display: 'block', marginBottom: 6 }}>6. घर/फ्लैट नंबर:</label>
                    <input type="text" value={houseNumber} onChange={e => setHouseNumber(e.target.value)} required placeholder="जैसे C-202" style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #0f0', background: '#111', color: '#0f0' }} />
                    <label style={{ display: 'block', marginBottom: 6, marginTop: 8 }}>लैंडमार्क (वैकल्पिक):</label>
                    <input type="text" value={landmark} onChange={e => setLandmark(e.target.value)} placeholder="जैसे चूना फाटक के पास" style={{ width: '100%', padding: 8, borderRadius: 4, border: '1px solid #0f0', background: '#111', color: '#0f0' }} />
                  </div>
                )}
                ...
                {gps && selectedLocality && selectedStreet && houseNumber && (
                  <div style={{ marginBottom: 14, color: '#0ff', fontWeight: 'bold' }}>
                    पूरा पता: {houseNumber}, {selectedLocality}, {selectedStreet}{landmark ? `, ${landmark}` : ''}
                  </div>
                )}
              </>
            )}
            ...
            {date !== null && selectedSlot !== null && gps && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', marginBottom: 8, color: '#0f0' }}>7. सफाई एजेंट चुनें:</label>
                  ...
                  {mockAgents
                    ...
                    .map(agent => (
                      <div key={agent.name} onClick={() => setSelectedAgent(agent.idx)} style={{ cursor: 'pointer', border: selectedAgent === agent.idx ? '2px solid #0f0' : '2px solid #333', borderRadius: 8, padding: 12, background: '#111', color: '#0f0', boxShadow: selectedAgent === agent.idx ? '0 0 8px #0f0' : 'none', textAlign: 'center', width: 120 }}>
                        ...
                        <div style={{ fontSize: 12, color: '#aaa' }}>उपलब्ध</div>
                      </div>
                    ))}
                </div>
              </div>
            ...
            {date !== null && selectedSlot !== null && gps && selectedAgent !== null && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', marginBottom: 6 }}>8. भुगतान:</label>
                <button type="button" style={{ width: '100%', background: '#0f0', color: '#111', border: 'none', borderRadius: 6, padding: '10px 0', fontWeight: 'bold', fontSize: 16, cursor: 'pointer', marginBottom: 8 }}>Razorpay से भुगतान करें (प्लेसहोल्डर)</button>
                <button type="submit" style={{ width: '100%', background: '#0f0', color: '#111', border: 'none', borderRadius: 6, padding: '12px 0', fontWeight: 'bold', fontSize: 18, cursor: 'pointer', marginTop: 8 }}>बुकिंग की पुष्टि करें</button>
              </div>
            )}
          </form>
        )
        ...
        agentSuccess ? (
          <div style={{ color: '#0f0', textAlign: 'center', fontWeight: 'bold', fontSize: 18 }}>
            उपलब्धता/सत्र(ों) की पुष्टि हुई!<br />आपको बुकिंग के लिए सूचित किया जाएगा।<br /><br />
            <button style={{ marginTop: 16, background: '#0f0', color: '#111', border: 'none', borderRadius: 6, padding: '8px 18px', fontWeight: 'bold', cursor: 'pointer', fontSize: 16 }} onClick={() => setAgentSuccess(false)}>और पोस्ट करें</button>
          </div>
        ) : (
          <form onSubmit={e => { e.preventDefault(); setAgentSuccess(true); }}>
            ...
            {agentDate && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', marginBottom: 8, color: '#0f0' }}>2. सत्र(ों) चुनें:</label>
                ...
                <div style={{ fontSize: 12, color: '#aaa', marginTop: 4 }}>(कई चुनने के लिए Ctrl/Cmd दबाए रखें)</div>
              </div>
            )}
            {/* Step 3: Locality Multi-Select */}
            {agentDate && agentSlots.length > 0 && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', marginBottom: 8, color: '#0f0' }}>3. इलाके चुनें (जहां आप सेवा दे सकते हैं):</label>
            ...
            {agentDate && agentSlots.length > 0 && agentLocalities.length > 0 && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', marginBottom: 8, color: '#0f0' }}>4. उपलब्ध कार्य सत्र:</label>
                {mockWorkSessions.filter(ws =>
                  ...
                  <div style={{ color: '#aaa', fontSize: 15, marginBottom: 8 }}>
                    कोई सत्र उपलब्ध नहीं। <b>इन स्लॉट के लिए अपनी उपलब्धता पोस्ट करें</b>।<br />
                    <button type="submit" style={{ marginTop: 8, background: '#0f0', color: '#111', border: 'none', borderRadius: 6, padding: '8px 18px', fontWeight: 'bold', cursor: 'pointer', fontSize: 16 }}>उपलब्धता पोस्ट करें</button>
                  </div>
                ) : (
                  <>
                    {mockWorkSessions.filter(ws =>
                      ...
                      <div key={ws.id} style={{ border: '2px solid #0f0', borderRadius: 8, padding: 12, marginBottom: 10, background: '#111', color: '#0f0', boxShadow: '0 0 8px #0f0' }}>
                        ...
                        <span style={{ color: '#aaa', fontSize: 13 }}>सत्र: {slots[ws.slot].label} {ws.date.toLocaleDateString('en-IN')} को</span><br />
                        <span style={{ color: '#aaa', fontSize: 13 }}>पता: {ws.house}, {ws.locality}, {ws.street}{ws.landmark ? `, ${ws.landmark}` : ''}</span><br />
                        <span style={{ color: '#aaa', fontSize: 13 }}>GPS: {ws.gps}</span><br />
                        <span style={{ color: '#0ff', fontWeight: 'bold' }}>₹{ws.price}</span>
                      </div>
                    ))}
                    <button type="submit" style={{ marginTop: 8, background: '#0f0', color: '#111', border: 'none', borderRadius: 6, padding: '8px 18px', fontWeight: 'bold', cursor: 'pointer', fontSize: 16 }}>चयनित सत्र स्वीकार करें</button>
                  </>
                )}
              </div>
            )}
