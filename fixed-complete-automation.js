...
async function step5NavigateToMetaAIAndFindPlusButton(page) {
  console.log('🔐 Step 5: Navigating to Meta AI and finding + button...');
  
  try {
    // Navigate to Meta AI PWA
    await page.goto(META_AI_PWA_URL, { waitUntil: 'networkidle0' });
    await delay(8000); // Wait longer for page to load
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      console.log(`🔍 Attempt ${attempt}: Looking for + button...`);
      ...
      for (const button of alternativeButtons) {
        ...
        if (ariaLabel && (ariaLabel.includes('+') || ariaLabel.includes('Add') || ariaLabel.includes('Upload'))) {
          console.log(`✅ Found alternative + button: ${ariaLabel}`);
          return true;
        }
        
        if (textContent && (textContent.includes('+') || textContent.includes('Add'))) {
          console.log(`✅ Found alternative + button: ${textContent}`);
          return true;
        }
      }
      
      console.log(`❌ Attempt ${attempt}: + button not found, refreshing...`);
...
async function step6UploadImageAndPrompt(page, imagePath) {
  ...
  try {
    ...
    if (plusButton) {
      ...
      console.log('❌ + button not found for upload');
      return false;
    }
...
async function processSingleImage(page, imagePath, imageNumber) {
  ...
  // Step 6: Upload image and add prompt (includes clicking + button)
...
async function main() {
  console.log('🚀 Starting FIXED COMPLETE Meta AI Automation...');
  console.log('🎯 Following the exact 13-step process with fixes!');
  ...
  try {
    ...
    // Step 5: Navigate to Meta AI and find + button
    const plusButtonFound = await step5NavigateToMetaAIAndFindPlusButton(page);
    if (!plusButtonFound) {
      console.log('❌ Could not find + button, stopping automation');
      return;
    }
    ...
    if (processedImages.length > 0) {
      ...
      console.log(`\n🎉 FIXED COMPLETE Automation finished!`);
