...
async function loginAndWaitForChat(page) {
  console.log('🔐 Login and wait for chat interface...');
  ...
  try {
    ...
    // Wait for redirect and chat interface
    console.log('⏳ Waiting for chat interface to load...');
    await delay(20000); // Wait 20 seconds for everything to load
    
    // Navigate to Meta AI and wait for chat
    await page.goto(META_AI_URL, { waitUntil: 'networkidle0' });
    await delay(10000); // Wait 10 seconds for chat interface
    
    console.log('✅ Login completed, waiting for chat interface...');
    return true;
...
async function waitForChatInterface(page) {
  console.log('🔍 Waiting for chat interface to appear...');
  
  // Wait up to 60 seconds for chat interface
  for (let i = 0; i < 12; i++) {
    console.log(`Attempt ${i + 1}: Checking for chat interface...`);
    ...
    const chatElements = await page.$$('[data-testid*="chat"], [data-testid*="input"], .chat, .input');
    if (chatElements.length > 0) {
      console.log('✅ Chat elements found!');
      return true;
    }
    
    console.log(`❌ Attempt ${i + 1}: Chat interface not found, waiting...`);
    await delay(5000);
  }
  
  console.log('❌ Chat interface not found after 60 seconds');
  return false;
}

async function findPlusButton(page) {
  console.log('🔍 Looking for + button in chat interface...');
  
  // Wait for chat interface first
  const chatFound = await waitForChatInterface(page);
  if (!chatFound) {
...
async function main() {
  console.log('🚀 Starting CHAT INTERFACE Meta AI Automation...');
  ...
  try {
    ...
    // Login and wait for chat interface
    const loginSuccess = await loginAndWaitForChat(page);
    ...
    if (loginSuccess) {
      ...
      if (plusButtonFound) {
        ...
        console.log(`\n🎉 CHAT INTERFACE Automation completed!`);
