...
async function noStallLogin(page) {
  console.log('🔐 NO-STALL login process...');
  ...
  try {
    ...
    // Step 1: Click login - NO WAITING
    console.log('🔍 Step 1: Clicking login...');
    await page.evaluate(() => {
    ...
    await delay(3000); // Wait a bit for page to change
    
    // Step 2: Click Continue with Instagram - NO WAITING
    console.log('🔍 Step 2: Clicking Continue with Instagram...');
    await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button, a'));
    ...
    await delay(5000); // Wait longer for Instagram page to load
    
    // Step 3: Fill credentials - NO WAITING
    ...
    // Step 4: Click submit - NO WAITING
    ...
    // Step 5: Wait for redirect and continue - NO STALLING
    console.log('⏳ Step 5: Waiting for redirect (no stalling)...');
    await delay(15000); // Wait 15 seconds and continue regardless
    
    console.log('✅ NO-STALL login process completed');
    return true;
...
async function processImageNoStall(page, imagePath, imageNumber) {
  console.log(`🎨 Processing image ${imageNumber}: ${path.basename(imagePath)}`);
  
  try {
    // Find and click + button - NO STALLING
    ...
    // Wait for file chooser and select file - NO STALLING
    ...
    // Find chat input and send prompt - NO STALLING
    ...
    // Download the generated image - NO STALLING
    const downloadSuccess = await downloadImageNoStall(page, imagePath, imageNumber);
...
async function downloadImageNoStall(page, originalImagePath, imageNumber) {
  try {
...
async function main() {
  console.log('🚀 Starting NO-STALL Meta AI Automation...');
  console.log('🎯 This version NEVER stalls - keeps moving forward!');
  ...
  try {
    ...
    // NO-STALL login process
    const loginSuccess = await noStallLogin(page);
    console.log(`Login result: ${loginSuccess ? 'Success' : 'Failed but continuing'}`);
    
    // Give time for page to load - NO STALLING
    console.log('⏳ Waiting 20 seconds for page to fully load (no stalling)...');
    await delay(20000);
    
    // Process images - NO STALLING
    ...
    for (let i = 0; i < miinaImages.length; i++) {
      ...
      const success = await processImageNoStall(page, imagePath, i + 1);
      if (success) {
        processedCount++;
